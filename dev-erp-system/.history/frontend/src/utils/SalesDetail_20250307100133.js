...
const SalesDetail = ({ isParentHide, v_treeName, show, onHide, listData, v_modalPropsData, v_propsSaleList, setSalesDetailData, isDeleted }) => {
    ...
    */
    ...
    const onSelect = (type) => (selectedKeys, info) => {
        ...
        const selectLinkedRadio = { ...info.node.title.props };
        if (info.selected === false) {

            // children 배열이 있고, 첫 번째 요소의 props가 존재하는 경우
            if (selectLinkedRadio.children?.[0]?.props) {
                // 조건이 맞으면 checked 값을 변경 (true -> false / false -> true)
                if (classiCode === selectLinkedRadio.children[0].props["data-code"]) {
                    selectLinkedRadio.children[0] = {
                        ...selectLinkedRadio.children[0],
                        props: {
                            ...selectLinkedRadio.children[0].props,
                            checked: !selectLinkedRadio.children[0].props.checked, // ✅ checked 값 토글
                        },
                    };
                }
            }
            // 이제 변경된 updatedProps를 상태 업데이트
            setInfo((prevInfo) => ({
                ...prevInfo,
                node: {
                    ...prevInfo.node,
                    title: {
                        ...prevInfo.node.title,
                        props: selectLinkedRadio, // 업데이트된 props 반영
                    },
                },
            }));
        }
       ...
       /*  if (info.selected) {
            if (classiCode === selectLinkedRadio?.children[0].props?.["data-code"]) {
                // info.node.title.props?.children[0].props?.value = false;
                console.log(selectLinkedRadio?.children[0].props)
                // console.log(info.node, info.node.title.props?.children[0].props?.["data-code"]);
            }
        } */
