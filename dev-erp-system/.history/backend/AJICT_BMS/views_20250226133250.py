...
def f_select_biz_opp1(request):
    ...
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({'STATUS': 'JSON', 'MESSAGE': 'JSON의 format이 틀립니다.'}, safe=False)

    if not v_session_user_id:
        return JsonResponse({'STATUS': 'FAIL', 'MESSAGE': 'a_session_user_id를 전달 받지 못했습니다.'}, safe=False)
    ...
    try:
        ...
        if not v_data_session:
            return JsonResponse({'STATUS': 'FAIL', 'MESSAGE': '해당 사용자가 존재하지 않습니다.'}, safe=False)
        ...
        # 조직 조회
        queries = {
            "search_headquarters": """SELECT * FROM ajict_bms_schema.dept WHERE LENGTH(dept_id) = 5 AND delete_date IS NULL ORDER BY dept_id""",
            "search_team": """SELECT * FROM ajict_bms_schema.dept WHERE LENGTH(dept_id) = 4 AND delete_date IS NULL ORDER BY dept_id""",
            "search_commonness_pro": """SELECT * FROM ajict_bms_schema.commonness_code WHERE great_classi_code = 'PRO' AND delete_date IS NULL ORDER BY small_classi_code"""
        }

        for key, sql in queries.items():
            with connection.cursor() as v_cursor:
                v_cursor.execute(sql)
                v_columns = [v_column[0] for v_column in v_cursor.description]
                v_rows = v_cursor.fetchall()
                v_data[key] = [dict(zip(v_columns, row)) for row in v_rows]

        # 비즈니스 기회 조회
        v_sql_biz_opp = """SELECT 
                              A.biz_opp_id,
                              A.biz_opp_name,
                              B.detail_no,
                              B.user_id,
                           ...
                           JOIN ajict_bms_schema.biz_opp_detail B ON A.biz_opp_id = B.biz_opp_id
                           LEFT JOIN ajict_bms_schema.aj_user AA ON B.user_id = AA.user_id AND AA.delete_date IS NULL
                           LEFT JOIN ajict_bms_schema.commonness_code BB ON BB.great_classi_code = B.last_client_com1_code AND BB.delete_date IS NULL
                           LEFT JOIN ajict_bms_schema.commonness_code CC ON CC.great_classi_code = B.last_client_com1_code 
                                                                        AND CC.small_classi_code = B.last_client_com2_code 
                                                                        AND CC.delete_date IS NULL
                           LEFT JOIN ajict_bms_schema.commonness_code NN ON NN.great_classi_code = A.progress1_rate_code AND NN.delete_date IS NULL
                           LEFT JOIN ajict_bms_schema.commonness_code OO ON OO.great_classi_code = A.progress1_rate_code 
                                                                        AND OO.small_classi_code = A.progress2_rate_code 
                                                                        AND OO.delete_date IS NULL
                           LEFT JOIN ajict_bms_schema.dept KK ON KK.dept_id = B.change_preparation_dept_id AND KK.delete_date IS NULL
                           LEFT JOIN ajict_bms_schema.dept MM ON MM.dept_id = KK.high_dept_id AND MM.delete_date IS NULL
                           LEFT JOIN ajict_bms_schema.biz_opp_detail_sale PP ON PP.biz_opp_id = B.biz_opp_id 
                                                                           AND PP.detail_no = 1 
                                                                           AND PP.delegate_tf = TRUE 
                                                                           AND PP.great_classi_code = 'BIZ'
                           LEFT JOIN ajict_bms_schema.biz_opp_detail_sale QQ ON QQ.biz_opp_id = B.biz_opp_id 
                                                                           AND QQ.detail_no = 1 
                                                                           AND QQ.delegate_tf = TRUE 
                                                                           AND QQ.great_classi_code = 'COR'
                           LEFT JOIN ajict_bms_schema.commonness_code RR ON RR.great_classi_code = 'BIZ' 
                                                                        AND RR.small_classi_code = PP.small_classi_code
                           LEFT JOIN ajict_bms_schema.commonness_code SS ON SS.great_classi_code = 'COR' 
...
"""
        ...
        with connection.cursor() as v_cursor:
           ...
           v_columns = [v_column[0] for v_column in v_cursor.description]  # 올바른 문법
           v_rows = v_cursor.fetchall()
        ...
        return JsonResponse({"STATUS": "FAIL", "MESSAGE": f"DB 오류: {str(db_error)}"}, safe=False)

    except Exception as e:
        print(f"❌ Unknown Error: {str(e)}")
        print(traceback.format_exc())  # 오류 상세 출력
        return JsonResponse({"STATUS": "FAIL", "MESSAGE": f"서버 오류: {str(e)}"}, safe=False)
   ...
   if not v_session_user_id:
      ...
      try:
         ...
         with transaction.atomic():
            ...
            # if v_sale_profit < 1:
            #    transaction.set_rollback(True)
            #    v_return = {'STATUS':'FAIL','MESSAGE':"'매출 이익' 항목의 값은 0보다 커야 합니다!"}
            #    v_square_bracket_return = [v_return]
            #    return JsonResponse(v_square_bracket_return,safe = False,json_dumps_params = {'ensure_ascii':False})
            ...
            # if v_purchase_amt < 1:
            #    transaction.set_rollback(True)
            #    v_return = {'STATUS':'FAIL','MESSAGE':"'매입 금액' 항목의 값은 0보다 커야 합니다!"}
            #    v_square_bracket_return = [v_return]
            #    return JsonResponse(v_square_bracket_return,safe = False,json_dumps_params = {'ensure_ascii':False})
