    ...
    useEffect(()=> {
        ...
        */
        if (show === false) {
            ...
            if (isSave === false) {
                console.log('isSave false');
                // case1
                if (isParentHide === true) {
                    console.log('isParentHide true');
                    setIsSave(false);
                    setSumBiz(null);
                    setSumCor(null);
                    ...
                    console.log(v_propsSaleData)
                    if (v_propsSaleData.length !== 0) {
                        console.log('propsData X');
                        setIsSave(false);
                        setSumBiz(null);
                        setSumCor(null);
                        setInputValues({biz: {}, cor: {}, a_product_name: ''});
                        ...
                        console.log('props는 있고 show, isSave는 false / parentHide는 true');
                    }
                }
            } else { //case2
                return;
        ...
        /* // 이 모달이 '닫기' 버튼을 클릭하여 닫혔을 경우
        if (show === false && isSave === false) {
            // 1. 부모 모달이 닫혔을 경우 완전히 초기화
            if (isParentHide === true) {
                setIsSave(false); 
                setVPropsSaleData([]);
            } else {
            }
            // 디비에 저장된 이전 데이터가 없는 경우
            // case 1. 사업 복제
            // case 2. 사업 신규 등록
            if (v_propsSaleData.length === 0) {
                setSumBiz(null);
