    ...
    const transformData = (data) => {
        ...
        if (data.length > 0) { 
            data.forEach((value) => {
                ...
                // biz, cor 내부 객체 순회
                value.forEach(([small_classi_code, arr]) => {
                    console.log(small_classi_code, arr);
                    const [a, sale_amt, delegate_tf] = arr;

                    const lower = value.great_classi_code.toLowerCase(); // great_classi_code 대체
                    if (!result[lower]) {
                        result[lower] = {}; //  키가 없으면 초기화
                    }

                    result[lower][small_classi_code] = [a, sale_amt, delegate_tf]; //  값 추가
                });
            });
        }
