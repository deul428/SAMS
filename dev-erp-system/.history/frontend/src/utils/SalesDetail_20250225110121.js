 ...
 */
    ...
    useEffect(() => {
        // 특정 타입(biz 또는 cor)의 합계를 계산하는 함수
        const calculateTotal = (classiData) => {
            console.log(classiData)
            return Object.values(inputValues[classiData])
                .filter(value => Array.isArray(value)) // 배열인 항목만 필터링
                .reduce((acc, cur) => acc + cur[1], 0); // 두 번째 요소(금액)를 합산
        };
        ...
        console.log(inputValuesRef.current, 'inputValues: ', inputValues, '\nbizTotal: ', bizTotal, '\ncorTotal: ', corTotal);
    }, [inputValues]);
    // --------------------- input value 합산 끝 ---------------------  
