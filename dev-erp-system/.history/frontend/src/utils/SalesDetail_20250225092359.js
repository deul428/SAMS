...
const SalesDetail = ({ v_treeName, show, onHide, listData, v_modalPropsData, setSalesDetailData }) => {
    // =================== 렌더 시 세팅 ===================  
    const [prevSaleAmt, setPrevSaleAmt] = useState(null);
    useEffect(() => {
        if (v_modalPropsData) { 
            setPrevSaleAmt(Number(v_modalPropsData.sale_amt)); 
        }
    }, [v_modalPropsData]);
...
/*     // UI에 total 금액 표현
    ...
    useEffect(() => {
                                    ...
                                    <div className="inputField">
                                        ...
                                        <Row className="cntntArea mb-2">
                                            <Col xs={12} md={12} lg={12} xl={12} className='col d-flex align-items-center floating'>
                                                <h3>총 매출 금액 &#40;현재 값&#41;: &#65510;{prevSaleAmt ? prevSaleAmt.toLocaleString('ko-KR') : ''}</h3>
                                            </Col>
        ...
        updateUI();
    }, [listData, bizTreeData, corTreeData, show, onHide, /* selectedBizKeys */, prevSaleAmt, sumBiz, sumCor, /* totalSaleAmt */]);
