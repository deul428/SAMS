 ...
 */
    ...
    const transformData = (data) => {
        ...
        if (data.length > 0) { 
            data.forEach((value) => {
                ...
                console.log(value[0].great_classi_code, value[0].small_classi_code)
                const lower = value.great_classi_code ? value.great_classi_code.toLowerCase() : null; 
                if (!lower) { return; }
                if (!result[lower]) {
                    result[lower] = {};
                }

                const small_classi_code = value.small_classi_code;
                if (!small_classi_code) {
                    console.warn("small_classi_code가 없음:", value);
                    return;
                }
                const sale_amt = value.sale_amt;
                const delegate_tf = value.delegate_tf;
                result[lower][small_classi_code] = [value.biz_opp_id, sale_amt, delegate_tf];
                console.log([value.biz_opp_id, sale_amt, delegate_tf]);
            });
        }
    ...
    const saveData = () => {
        ...
        let transformedData;
        if (v_propsSaleData) {
            transformedData = transformData(v_propsSaleData);
        } else {
            transformedData = {};
        }
