...
/* 
    ...
    const treeRender = (listData, propsData, type) => {
        ...
        if (inputValues.biz || inputValues.cor) {
            ...
            inputArr.map((e, index) => {
                if(e[0] === 'biz' || e[0] === 'cor'){
                    ...
                    const { code, amt, delegateFlag } = arr;
                    // console.log(delegateFlag);

                }
            })
        }
        console.log(Object.values(inputValues),propsData);
        // const inputMap = new Map(inputValues.map(e => [e.small_classi_code, { sale_amt: e.sale_amt, delegate_tf: e.delegate_tf }]));
        
        return listData.map((e, index) => {
            const matchedData = propsMap.get(e.small_classi_code) || { sale_amt: 0, delegate_tf: false };
            const inputData = propsMap.get(e.small_classi_code) || { sale_amt: 0, delegate_tf: false };
            console.log(inputValues[e.great_classi_code.toLowerCase()][e.small_classi_code]?.[2]);
                    ...
                    <div className="titleArea"
                        ...
                        <input 
                            ...
                            defaultChecked={isSelected
                                ? inputValues[e.great_classi_code.toLowerCase()][e.small_classi_code]?.[2] ?? matchedData.delegate_tf : false}
