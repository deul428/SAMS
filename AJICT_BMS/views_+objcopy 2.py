   ...
   #   return JsonResponse(v_square_bracket_return,safe=False,json_dumps_params = {'ensure_ascii':False})
      ...
      try:
         ...
         with connection.cursor() as v_cursor_biz_opp:
            ...
            # 데이터 구조를 객체 형식으로 설정
            v_data_biz_opp = {
               "retrieve_biz_opp": [dict(zip(v_columns_biz_opp, row)) for row in v_rows_biz_opp]
            }

            # v_data_biz_opp를 배열이 아닌 객체로 설정
            v_data = [v_data_biz_opp]  # v_data는 이제 배열 형태로, 내부에 객체가 포함됨
            v_square_bracket_data = v_data
