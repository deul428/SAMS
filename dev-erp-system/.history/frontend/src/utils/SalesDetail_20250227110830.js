        ...
        if (data.length > 0) { 
            data.forEach((value) => {
                ...
                value.map((e) => {
                    ...
                    const lower = e.great_classi_code ? e.great_classi_code.toLowerCase() : null; 
                    if (!lower) { return; }
                    if (!result[lower]) {
                        result[lower] = {};
                    }
                    ...
                    result[lower][small_classi_code] = [e.biz_opp_id, sale_amt, delegate_tf];
                    console.log([e.biz_opp_id, sale_amt, delegate_tf]);
                })
            });
        }
