  ...
  login: {depth1: 'login', depth2: '', component: AuthLogin, endpoint: 'login/'},
    ...
    bizoppSelect1: {depth1: 'biz-opp', depth2: '', endpoint: 'select-biz-opp1/', component: BizOpp, 
      props: [
        ...
        Cell: ({value}) => v_handlingDate(value) },
        { Header: '총 매출 금액', accessor: 'total_sale_amt', 
    ...
    bizoppHistory: {depth1: 'biz-opp-history', depth2: '', endpoint: 'select-biz-opp-history/', component: BizOppHistory,
      props: [
        ...
        { Header: '갱신 구분', accessor: 'renewal_code', 
          Cell: ({value}) => 
            <div className='tableCell'>
              {value === 'I'? value = '등록' : value === 'U'? value = '수정' : value === 'D'? value = '삭제' : value === 'C'? value = '등록(복제)' : '' }
            </div> 
        },
        { Header: '담당자', accessor: 'user_name',
          Cell: ({value}) => (<div className='tableCell'>{value}</div>)
        ...
        { Header: '대표 여부', accessor: 'delegate_tf', 
          Cell: ({ value, row }) => 
            (<div className='tableCell'>{v_handlingTF(value)}</div>)
          /* (
