    ...
    */
    ...
    const onSelect = (type) => (selectedKeys, info) => {
        ...
        setInputValues((prev) => {
            ...
            if (info.selected) {
                updatedType[classiCode] = Array.isArray(updatedType[classiCode])
                    ? [classiName, updatedType[classiCode][1], updatedType[classiCode][2] ?? false]
                    : [classiName, 0, false];
            } else {
                delete updatedType[classiCode];
            }
 ...
 */
    const handleInputChange = (e, type, classiCode, isRadio = false) => {
        ...
        setInputValues((prev) => {
            ...
            const updatedClassiData = { ...prev[type] };
            const classiInfo = updatedClassiData[classiCode];
            console.log(classiInfo)
    
            // input radio
            if (isRadio) {
                // 모든 기존 항목을 false로 변경 (라디오 버튼 단일 선택 유지)
                Object.keys(updatedType).forEach((prevClassiCode) => {
                    console.log(updatedType[prevClassiCode])
                    updatedType[prevClassiCode] = Array.isArray(updatedType[prevClassiCode])
                        ? [updatedType[prevClassiCode][0], updatedType[prevClassiCode][1], false]
                        : [updatedType[prevClassiCode][0], 0, false];
                });

                // 선택된 항목만 true로 설정
                updatedType[classiCode] = Array.isArray(updatedType[classiCode])
                    ? [updatedType[classiCode][0], updatedType[classiCode][1], true]
                    : [classiName, 0, true];
            } else { 
                // input radio X(number): 입력 시 number만 업데이트 유지
                updatedType[classiCode] = Array.isArray(updatedType[classiCode])
                    ? [updatedType[classiCode][0], Number(e.target.value), updatedType[classiCode][2] ?? false]
