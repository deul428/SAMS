...
const SalesDetail = ({ v_treeName, show, onHide, listData, v_modalPropsData, v_propsSaleData, setSalesDetailData }) => {
    ...
    */
    ...
    const treeRender = (listData, propsData, type) => {
        const propsMap = new Map(propsData.map(item => [item.small_classi_code, item.sale_amt]));
            ...
            title: (
                <div className="titleArea"
                    ...
                    <input 
                        type="radio"
                        name={`radio-${type}`}
                        // value={e.small_classi_code}
                        data-key={e.small_classi_name}
                        // defaultChecked={true}
                        // defaultValue={true}
                        // checked={inputValues[type]?.[e.small_classi_code]?.[1] ?? false}
                    ...
                    <div className={`${index} titleItem`} data-key={`${type}-${index}`}>
                        ...
                        <input
                            type="number"
                            placeholder="세부 금액"
                            defaultValue={propsMap.get(e.small_classi_code) ?? 0}
                            data-key={e.small_classi_name}
                            // value={inputValues[type]?.[e.small_classi_code]?.[0] ?? 0}
