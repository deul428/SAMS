...
const roots = {
  login: {depth1: 'login', depth2: '', component: AuthLogin, endpoint: 'login/'},
    ...
    props: [
      ...
      { Header: '갱신 일시', accessor: 'renewal_date', Cell: ({value}) => value ? moment(value).format('YYYY-MM-DD HH:mm') : '' },
      ...
      /* Cell: ({ value, row }) => (
        <div style={dynamicCellStyle(row, 'history_assistance_no')} className='tableCell'>
          {applyFunctions(value, v_handlingNum)}
        </div>
        ),  */ 
      },
      ...
      /* Cell: ({ value, row }) => (
        <div style={dynamicCellStyle(row, 'great_classi_code')} className='tableCell'>
          {applyFunctions(value, v_handlingNum)}
        </div>
        ),   */
      }, 
      ...
      /* Cell: ({ value, row }) => (
        <div style={dynamicCellStyle(row, 'small_classi_code')} className='tableCell'>
          {applyFunctions(value, v_handlingNum)}
        </div>
        ),   */
      }, 
      { Header: '매출 금액', accessor: 'small_classi_name', 
      /* Cell: ({ value, row }) => (
        <div style={dynamicCellStyle(row, 'great_classi_code')} className='tableCell'>
          {applyFunctions(value, v_handlingNum)}
        </div>
