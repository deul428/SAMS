...
const SalesDetail = ({ v_treeName, show, onHide, listData, v_modalPropsData, setSalesDetailData }) => {
    ...
    */
    ...
    const onSelect = (type) => (selectedKeys, info) => {
        ...
        setInputValues((prev) => {
            const classiData = prev[type] ? { ...prev[type] } : {};
            if (info.selected) {
                classiData[classiCode] = Array.isArray(classiData[classiCode])
                    ? [classiName, classiData[classiCode][1], classiData[classiCode][2] ?? false]
                    : [classiName, 0, false];
            } else {
                delete classiData[classiCode];
            }
 ...
 */
    const handleInputChange = (e, type, classiCode, isRadio = false) => {
        ...
        setInputValues((prev) => {
            ...
            const classiData = { ...prev[type] };
    
            // input radio
            if (isRadio) {
                // 모든 기존 항목을 false로 변경 (라디오 버튼 단일 선택 유지)
                Object.keys(classiData).forEach((prevClassiCode) => {
                    classiData[prevClassiCode] = Array.isArray(classiData[prevClassiCode])
                        ? [classiData[prevClassiCode][0], classiData[prevClassiCode][1], false]
                        : [classiData[prevClassiCode][0], 0, false];
                });

                // 선택된 항목만 true로 설정
                classiData[classiCode] = Array.isArray(classiData[classiCode])
                    ? [classiData[classiCode][0], classiData[classiCode][1], true]
                    : [classiName, 0, true];
            } else { 
                // input radio X(number): 입력 시 number만 업데이트 유지
                classiData[classiCode] = Array.isArray(classiData[classiCode])
                    ? [classiData[classiCode][0], Number(e.target.value), classiData[classiCode][2] ?? false]
