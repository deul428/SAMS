...
const SalesDetail = ({ isParentHide, v_treeName, show, onHide, listData, v_modalPropsData, v_propsSaleList, setSalesDetailData }) => {
    ...
    */
    ...
    useEffect(() => {
        ...
        } else if (isRecover === true) { //수정 모드에서, 작업 중인 데이터를 저장하지 않고 다시 진입했을 경우 v_propsSaleData를 수정 전 상태로 원복하기 위해 감지용 플래그로 조건 처리

        }
        console.log('v_propsSaleData가 변경되었음!', v_propsSaleData);
        ...
        */
        ...
        if (show === false && isSave === false) {
            ...
            if (!v_propsSaleList || v_propsSaleList.length === 0) {
                ...
                setVPropsSaleData(v_propsSaleList); // propsData 강제 리셋
                setIsRecover(true);
            }
        }
