    ...
    const transformData = (data) => {
        ...
        if (data.length > 0) { 
            data.forEach((value) => {
                console.log(value);
                // biz, cor 내부 객체 순회
                value.map(([small_classi_code, arr]) => {
                    const [small_classi_code, sale_amt, delegate_tf] = arr;

                    const lower = value.toLowerCase(); // great_classi_code 대체
                    if (!result[lower]) {
                        result[lower] = {}; //  키가 없으면 초기화
                    }

                    result[lower][small_classi_code] = [small_classi_code, sale_amt, delegate_tf]; //  값 추가
                });
            });
        }
