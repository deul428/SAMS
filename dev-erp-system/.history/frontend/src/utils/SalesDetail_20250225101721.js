...
const SalesDetail = ({ v_treeName, show, onHide, listData, v_modalPropsData, setSalesDetailData }) => {
    ...
    */
    ...
    const onSelect = (type) => (selectedKeys, info) => {
        ...
        // keyName 값: select된 요소의 small_classi_name
        const keyName = info.node?.title?.props?.["data-key"];
        const keyCode = info.node.small_classi_code;
        ...
        setInputValues((prev) => {
            ...
            if (info.selected) {
                // ✅ 기존 `boolean` 값 유지
                updatedType[keyCode] = Array.isArray(updatedType[keyCode])
                    ? [keyCode, updatedType[keyCode][1], updatedType[keyCode][2] ?? false]  // 기존 boolean 값 유지
                    : [keyCode, 0, false];  // 기본값 설정
            } else {
                delete updatedType[keyCode];
            }
    ...
    // 선택 해제된 key는 UI에서 0으로 유지. 이 코드가 없을 시 selected된 요소를 false했다 true할 시 inputValues 객체는 숫자가 0으로 초기화되지만 UI는 초기화되지 않음. inputValues 값 및 UI 표현 값을 보존하기 위해 사용.
    useEffect(() => {
        setInputValues((prev) => {
            return {
                ...prev,
        ...
        setInputValues((prev) => {
            ...
            if (isRadio) {
                ...
                updatedType[key] = Array.isArray(updatedType[key])
                    ...
                    : [keyName, Number(e.target.value), false];
            }
